...
class CategoriesFront extends Component {
    ...
    <div className="row">
{
    (loading) ? (<img className="preloader" src="/808.gif" alt="preloader" />) : items.length ? items.map((item, i) => {
    return <div key={i} className="col-lg-3 col-md-6 col-xs-12">
    ...
    <ul>
    {item.children_info.map((elem, i) => {
    let arr = elem.split('/');
    return <li key={i}><Link to={`/category?cat_id=${arr[1]}`}>{arr[0]}</Link><span className="category-counter">({arr[2]})</span></li>
})}
    </ul>
    </div>
    </div>
